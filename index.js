...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.commandName === 'ping') {
        ...
        // Проверка роли @everyone
        if (!guild.roles.everyone) {
            return interaction.reply('Ошибка: Роль @everyone не найдена на этом сервере.');
        }

        // Проверка подключения бота
        if (!guild.me) {
            return interaction.reply('Ошибка: Бот еще не подключился к этому серверу.');
        }
        ...
        await interaction.reply(`Создан ${channelType} канал ${channelName} для роли ${targetRole.name}. Ссылка: ${channel.url}`);
